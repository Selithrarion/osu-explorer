...
use crate::web::state::IndexingProgress;
use anyhow::Result;
...
pub fn run_indexing(
    ...
    let bar = ProgressBar::new(osu_files.len() as u64);
    bar.set_style(
    ...
    let results: Vec<(Vector, MapInfo)> = osu_files
        .par_iter()
        .filter_map(|entry| {
            bar.inc(1);
			if let Some(tracker) = &progress_tracker {
				if bar.position() % 100 == 0 {
					tracker.lock().unwrap().progress = bar.position();
				}
			}
    ...
    let (vectors, map_infos): (Vec<Vector>, Vec<MapInfo>) = results.into_iter().unzip();
